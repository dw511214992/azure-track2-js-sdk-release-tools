        ...
        (all: TshParameter[], cur: ParameterDeclaration) => {
            ...
            if (isIdentifier(cur.name)) {
                params.push(new TshParameter(
                    (cur.name as Identifier).text, getNodeType(cur.type), !!cur.questionToken, cur.getStart(), cur.getEnd(),
                ));
            } else if (isObjectBindingPattern(cur.name)) {
                const elements = cur.name.elements;
                let types: (string | undefined)[] = [];
                const boundParam = new ObjectBoundParameterDeclaration(!!cur.questionToken, cur.getStart(), cur.getEnd());
                ...
                const elements = cur.name.elements;
                let types: (string | undefined)[] = [];
                const boundParam = new ArrayBoundParameterDeclaration(!!cur.questionToken, cur.getStart(), cur.getEnd());
